...
| Full-replacement updates reject clients that send only changed fields | Medium | Require the mandatory fields in `UpdateProductoDto` (Product is a strict full replacement) and call out the stricter Product update contract in release notes. Catalog update DTOs remain partial — present invalid values and explicit `null` are rejected, but omission stays allowed — to preserve the existing catalog suites without modifying them. |
